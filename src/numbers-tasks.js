...
function isPowerOfTwo(num) {
  return num > 0 && num % 2 === 0 && Math.log2(num) % 1 === 0;
}
...
function numberToStringInBase(number, base) {
  if (typeof number !== 'number' || typeof base !== 'number') {
    throw new Error('Invalid input: number and base must be numbers');
  }
  let sign = '';
  let num = number;
  if (num < 0) {
  ...
  if (base < 2 || base > 36) {
    throw new Error('Invalid input: base must be between 2 and 36');
  }
  if (num === 0) return '0';
  const digits = [];
  while (num > 0) {
...
function toFixed(number, fractionDigits) {
  ...
  if (
    ...
    throw new Error(
      'Invalid input: fractionDigits must be a non-negative integer'
    );
  }
...
function toPrecision(number, precision) {
  ...
  const preciseNumber = number.toPrecision(precision);
